...
int32_t SendDynamicArea(uint8_t *Data, uint32_t Length)
{
	...
	{
		/*printf("%02X ", 0xA5);*/
          HAL_UART_Transmit(&huart3,buf,1,1000);
	}
	...
	for (i = 0; i < Length; i++)
	{
		...
		if (Data[i] == 0xA5)
		{
			/*printf("%02X %02X ", 0xA6, 0x02);*/
                  buf[0]=0xA6;
                  buf[1]=0x02;
                  HAL_UART_Transmit(&huart3,buf,2,1000);
		}
...
void Led5kSetScreenParam(void)
{
	...
	/*for (i = 0; i < sizeof(StartWriteScreenParamFileCMD); i++)
	{
        ...
        HAL_UART_Transmit(&huart3,(uint8_t *)StartWriteScreenParamFileCMD,sizeof(StartWriteScreenParamFileCMD),1000);
	...
	/*for (i = 0; i < sizeof(LED_SET_SCREEN_PARAM); i++)
	{
        ...
        HAL_UART_Transmit(&huart3,(uint8_t *)LED_SET_SCREEN_PARAM,sizeof(LED_SET_SCREEN_PARAM),1000);
