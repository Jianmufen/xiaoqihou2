...
{
  // Initialize the Memory Allocation System  ³õÊ¼»¯´æ´¢Æ÷
  osal_mem_init();
  ...
  // Initialize the Power Management System  ³õÊ¼»¯µçÔ´¹ÜÀíÏµÍ³
  osal_pwrmgr_init();

  // Initialize the system tasks. ³õÊ¼»¯ÈÎÎñ
  osalInitTasks();

  // Setup efficient search for the first free block of heap.ÉèÖÃµÚÒ»¸ö¿ÕµÄ¶ÑÕ»Îª¸ßÐ§ËÑË÷
  osal_mem_kick();
...
{
  uint8 idx = 0;//ÈÎÎñË÷ÒýºÅ

  osalTimeUpdate();  //É¨ÃèÄÄ¸öÊÂ¼þ±»´¥·¢ÁË£¬È»ºóÖÃÏàÓ¦µÄ±êÖ¾Î»
  Hal_ProcessPoll(); //´Ëº¯Êý´úÌæÁËMT_SerialPoll() and osal_check_timer()
  ...
  } while (++idx < tasksCnt);//µÃµ½´ý´¦ÀíµÄ×î¸ßÓÅÏÈ¼¶ÈÎÎñË÷ÒýºÅidx
  ...
  if (idx < tasksCnt)
  {
    ...
    HAL_ENTER_CRITICAL_SECTION(intState);//½øÈëÁÙ½çÇø£¬±£»¤
    events = tasksEvents[idx];   //ÌáÈ¡ÐèÒª´¦ÀíµÄÈÎÎñÊÂ¼þ
    tasksEvents[idx] = 0;  // Clear the Events for this task.Çå³ý±¾´ÎÈÎÎñÊÂ¼þ
    HAL_EXIT_CRITICAL_SECTION(intState);//ÍË³öÁÙ½çÇø

    activeTaskID = idx;  
    events = (tasksArr[idx])( idx, events );//Í¨¹ýÖ¸Õëµ÷ÓÃÈÎÎñ´¦Àíº¯Êý
    activeTaskID = TASK_NO_TASK;

    HAL_ENTER_CRITICAL_SECTION(intState);//½øÈëÁÙ½çÇø£¬±£»¤
    tasksEvents[idx] |= events;  // Add back unprocessed events to the current task.±£´æÎ´´¦ÀíµÄÊÂ¼þ
    HAL_EXIT_CRITICAL_SECTION(intState);//ÍË³öÁÙ½çÇø
  }
