 ...
 *   ÔÚNV·¢ÏÖitem ID and ·µ»ØÒ³ÊýºÃºÍÊý¾ÝµÄÆ«ÒÆÁ¿
 *
 * @param   id - Valid NV item Id.
 *
 * @return  Offset of data corresponding to item Id, if found;
 *    Èç¹û·¢ÏÖitem ID ·µ»ØÆ¥Åäitem IDµÄÆ«ÒÆÁ¿
 *          otherwise OSAL_NV_ITEM_NULL.·ñÔò·µ»ØOSAL_NV_ITEM_NULL
 *
...
uint8 osal_nv_item_init( uint16 id, uint16 len, void *buf )
{
  ...
  if ( !OSAL_NV_CHECK_BUS_VOLTAGE ) //¼ì²éÊÇ·ñÉÏµç
  {
    return NV_OPER_FAILED;
  }
  else if ((offset = findItem(id)) != OSAL_NV_ITEM_NULL)//Èç¹û·¢ÏÖÁËitem ID
  {
    // Re-populate the NV hot item data if the corresponding items are already established.
    //Èç¹ûÏàÓ¦µÄÏîÄ¿£¨item£©ÒÑ¾­±»½¨Á¢ÁË£¬ÄÇ¾ÍÖØÐÂÔÚNV¸üÐÂÏîÄ¿£¨item£©Êý¾Ý
