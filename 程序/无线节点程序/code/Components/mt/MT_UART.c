 ...
 * @return  None
 ***************************************************************************************************/
void MT_UartProcessZToolData ( uint8 port, uint8 event )//Íø·äÍ¸´«Ð­Òé
{
   uint8 flag=0,i,j=0; //flag ÊÇÅÐ¶ÏÓÐÃ»ÓÐÊÕµ½Êý¾Ý£¬ j ¼ÇÂ¼Êý¾Ý³¤¶È
   uint8 buf[128]; //´®¿Ú buffer ×î´ó»º³åÄ¬ÈÏÊÇ 128£¬ÎÒÃÇÕâÀïÓÃ 128.
   (void)event; // Intentionally unreferenced parameter
   while (Hal_UART_RxBufLen(port)) //¼ì²â´®¿ÚÊý¾ÝÊÇ·ñ½ÓÊÕÍê³É
   {
     HalUARTRead (port,&buf[j], 1); //°ÑÊý¾Ý½ÓÊÕ·Åµ½ buf ÖÐ
     j++; //¼ÇÂ¼×Ö·ûÊý
...
//ÖØ¶¨Òåputcharº¯ÊýÎª´®¿Ú0·¢ËÍ£¬¿ÉÒÔÊ¹ÓÃprintfº¯Êý½øÐÐ´®¿Ú·¢ËÍ£¬ÕâÖÖ·½Ê½ÊÇ×èÈûµÄ£¬¿ÉÒÔÔÚÎ´Á¬½ÓÉÏÍøÂç»òÎ´´ò¿ª¿´ÃÅ¹·Ç°Ê¹ÓÃ
__near_func int putchar(int c)
{
  WDT_Feed();
   
  U0DBUF=c;
  while(UTX0IF==0)   //µÈ´ý´«ËÍÍê³É¡£×¢ÒâÕâÀï²»ÄÜÓÃU0CSRµÄbit1¼´TX_BYTEÎ»ÅÐ¶Ï·¢ËÍ×´Ì¬£¬ÒªÓÃ·¢ËÍÖÐ¶Ï±êÖ¾UTX0IFÅÐ¶Ï
    WDT_Feed();
  UTX0IF=0;
