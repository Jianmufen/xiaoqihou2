...
void ZDApp_Init( uint8 task_id )
{
  ...
  ZDAppNwkAddr.addrMode = Addr16Bit;//µã²¥Ä£Ê½
  ZDAppNwkAddr.addr.shortAddr = INVALID_NODE_ADDR;//ÎÞÐ§½ÚµãµØÖ·
  ...
  // Start the device?
  if ( devState != DEV_HOLD )//devState³õÊ¼ÖµÎªDEV_INIT£¬¼´ÔÚ³õÊ¼»¯ZDA²ãÊ±£¬¾ÍÖ´ÐÐ¸ÃÌõÓï¾ä
  {
    ZDOInitDevice( 0 );//Éè±¸³õÊ¼»¯
  }
  else
  {
...
UINT16 ZDApp_event_loop( uint8 task_id, UINT16 events )
{
  ...
  if ( events & ZDO_NETWORK_INIT )  //ÍøÂç³õÊ¼»¯ÊÂ¼þ´¦Àí
  {
    // Initialize apps and start the network
    devState = DEV_INIT;
    osal_set_event( ZDAppTaskID, ZDO_STATE_CHANGE_EVT );

    //Éè±¸Âß¼­ÀàÐÍ£¬Æô¶¯Ä£Ê½£¬ÐÅ±êÊ±¼ä£¬³¬Ö¡³¤¶È£¬Æô¶¯Éè±¸
 ...
 *ÔÚÍøÂçÀïÆô¶¯Éè±¸£¬¸ù¾ÝZCD_NV_STARTUP_OPTION(NV item) ¾ö¶¨ÊÇ·ñ±£´æÉè±¸µÄÍøÂç×´Ì¬
 *
...
uint8 ZDOInitDevice( uint16 startDelay )
{
  ...
  if( ZDO_INIT_HOLD_NWK_START != startDelay )
  {
    ...
    // Trigger the network start  ÍøÂç³õÊ¼»¯
...
void ZDApp_NetworkInit( uint16 delay )
{
  if ( delay )
  {
    // Wait awhile before starting the device·¢ËÍZDO_NETWORK_INIT£¨ÍøÂç³õÊ¼»¯£©ÏûÏ¢µ½ZDApp²ã
