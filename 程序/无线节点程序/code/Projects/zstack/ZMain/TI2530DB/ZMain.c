...
//2017.4.10Ìí¼Ó³õÊ¼»¯NVÖÐÄÚÈÝµÄº¯ÊýnvItemInit()£¬ÓÃÓÚ½«Ä¬ÈÏChannelºÍPanID±£´æµ½NV£»´ÓZGlobals.cÖÐµÄ¾²Ì¬º¯ÊýzgItemInit()º¯ÊýÐÞ¸Ä¶øÀ´
/*********************************************************************
 * @fn       nvItemInit()
 *
 * @brief
 ...
 * @param   set - TRUE to set , or read
 *
 * @return  ZSUCCESS if successful, NV_ITEM_UNINIT if item did not
 *          exist in NV, NV_OPER_FAILED if failure.
 */
uint8 nvItemInit( uint16 id, uint16 len, void *buf, uint8 set );
uint8 nvItemInit( uint16 id, uint16 len, void *buf, uint8 set )
{
  uint8 status;

  // If the item doesn't exist in NV memory, create and initialize
  // it with the value passed in.Èç¹ûNVÃ»ÓÐÏîÄ¿£¨item£©¾Í½¨Á¢ºÍ³õÊ¼»¯ÏîÄ¿
  //¸ù¾Ý´«ÈëµÄ²ÎÊý
  status = osal_nv_item_init( id, len, buf );//NV¶ÁÐ´Ö®Ç°±ØÐëµ÷ÓÃÕâ¸öº¯Êý
  if ( status == ZSUCCESS )
  {
    if ( set )
    {
      // Write the default value back to NV Ð´Ä¬ÈÏÖµ
      status = osal_nv_write( id, 0, len, buf );//NVÐ´
    }
    else
    {
      // The item exists in NV memory, read it from NV memory
      status = osal_nv_read( id, 0, len, buf );//NV¶Á
    }
  }
...
int main( void )
{
  // Turn off interrupts ¹Ø±ÕËùÓÐÖÐ¶Ï
  osal_int_disable( INTS_ALL );

  // Initialization for board related stuff such as LEDs ³õÊ¼»¯°åÔØÍâÉè
  HAL_BOARD_INIT();

  // Make sure supply voltage is high enough to run  ¼ì²éÐ¾Æ¬µçÑ¹ÊÇ·ñÕý³£
  zmain_vdd_check();

  // Initialize board I/O  ³õÊ¼»¯IO¿Ú
  InitBoard( OB_COLD );

  // Initialze HAL drivers  ³õÊ¼»¯¸÷¸öÓ²¼þÇý¶¯
  HalDriverInit();

  // Initialize NV System  ³õÊ¼»¯Flash´æ´¢Æ÷
  osal_nv_init( NULL );

  // Initialize the MAC  ³õÊ¼»¯MAC²ã
  ZMacInit();

  // Determine the extended address  È·¶¨IEEE  64Î»µØÖ·
  zmain_ext_addr();
  ...
  // Initialize basic NV items ³õÊ¼»¯·ÇÒ×Ê§±äÁ¿
  zgInit();
  
  /* 2017.4.10Ìí¼ÓÐÞ¸ÄÈëÍøÊ±Ä¬ÈÏµÄchennelºÍpanid£¬²»´ò¿ªNV_INITºÍNV_RESTORE£»
     ½öÖÕ¶Ë½ÚµãÌí¼Ó£¬Ð­µ÷Æ÷»¹ÊÇÊ¹ÓÃNV_INITºÍNV_RESTORE±£³Ö²»±ä£¬ÒòÎªÐ­µ÷Æ÷ÐèÒª±£´æÍøÂçÐÅÏ¢ÈçÁÚ¾Ó±íµÈ£¬
     ÕâÑùÖÕ¶Ë½ÚµãÖØÐÂÈëÍøºóµØÖ·µÈÐÅÏ¢²»»á¸Ä±ä */
#if (!defined (ZDO_COORDINATOR)) && (!defined (RTR_NWK))  //ÖÕ¶Ë½Úµã
  
  //´ÓNVÖÐ¶ÁÈ¡ÐÅµÀºÍpanidÐÅÏ¢£¬ÐèÒª¹Ø±ÕNV_INITºÍNV_RESTOREÑ¡Ïî
  //ÐÅµÀ
  if((nvItemInit(ZCD_NV_CHANLIST,sizeof(zgDefaultChannelList), &zgDefaultChannelList,FALSE) != ZSUCCESS) ||   //zgDefaultChannelListÎªÄ¬ÈÏ¶¨ÒåµÄÐÅµÀ£¬²»Ê¹ÄÜNV_RESTOREÊ±ÒÔËüÎªÄ¬ÈÏÖµ½øÐÐÈëÍø
     (zgDefaultChannelList == 0xFFFFFFFF))
  {
    zgDefaultChannelList = DEFAULT_CHANLIST;   //Ê¹ÓÃ³õÊ¼ÅäÖÃÖµ
  }
  
  //panid
  if(nvItemInit(ZCD_NV_PANID,sizeof(zgConfigPANID), &zgConfigPANID,FALSE) != ZSUCCESS)   //zgConfigPANIDÎªÄ¬ÈÏ¶¨ÒåµÄpanid£¬²»Ê¹ÄÜNV_RESTOREÊ±ÒÔËüÎªÄ¬ÈÏÖµ½øÐÐÈëÍø
  {
    zgConfigPANID = ZDAPP_CONFIG_PAN_ID;   //Ê¹ÓÃ³õÊ¼ÅäÖÃÖµ
  }
  ...
  // Allow interrupts  ´ò¿ªËùÓÐÖÐ¶Ï
  osal_int_enable( INTS_ALL );

  // Final board initialization  ³õÊ¼»¯°åÔØÍâÉè
  InitBoard( OB_READY );

  // Display information about this device  ÏÔÊ¾Ïà¹ØÐÅÏ¢
  zmain_dev_info();
  ...
  osal_start_system(); // No Return from here Ö´ÐÐ²Ù×÷ÏµÍ³£¬½øÈ¥ºó²»»á·µ»Ø
